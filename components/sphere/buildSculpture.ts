...
// Shared by the on-screen R3F preview and the offscreen depth/control export,
// so what you see and what conditions the model are the same form.

import * as THREE from "three";
import type { TreeNode } from "@/lib/types";
import { seedFromSignature, stableSignature } from "@/lib/hash";
import { gestureFromBaseline } from "@/lib/translation/gesture";
import { createSeededNoise, fbm } from "./seededNoise";
import { gestureTubeGeometry } from "./gestureCurve";

export interface Sculpture {
  bodyGeometry: THREE.BufferGeometry;
  gestureGeometry: THREE.BufferGeometry;
  dispose(): void;
}

/** Map geometry descriptor keywords → displacement modifiers. */
function geometryModifiers(keywords: string[]) {
  let amplitude = 1;
  let frequency = 1;
  let detail = 0;
...
export function buildSculpture(baselinePath: TreeNode[]): Sculpture {
  ...
  // Geometry descriptors come from the baseline lane + its modulators.
  // route() needs a state path, but geometry must ignore state entirely —
  // pass a minimal stand-in and use only the form-side output.
...
function displace(geo: THREE.BufferGeometry, f: (x: number, y: number, z: number) => number) {
