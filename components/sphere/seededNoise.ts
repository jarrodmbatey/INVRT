...
export interface Noise3D {
  (x: number, y: number, z: number): number; // [-1, 1]
}

export function createSeededNoise(seed: number): Noise3D {
  ...
  const lattice = (xi: number, yi: number, zi: number) => {
...
export function fbm(noise: Noise3D, x: number, y: number, z: number, octaves = 4): number {
  let value = 0;
  let amplitude = 0.5;
  let frequency = 1;
