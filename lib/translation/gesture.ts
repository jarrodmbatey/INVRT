...
import type { EnergyWord, Gesture, GestureFamily, TreeNode } from "../types";
import { mulberry32, seedFromSignature } from "../hash";
import { baselineTree } from "./baselineTree";
import { pathIndices } from "./trees";
import { gestureWordByFamily } from "./vocabulary";
...
export function gestureFromBaseline(baselinePath: TreeNode[], signature: string): Gesture {
  const family: GestureFamily = baselinePath[0].descriptors?.pathBias ?? "contour";
...
export function gestureEnergyWord(energy: EnergyWord): string {
  switch (energy) {
    case "rising":
