 ...
 */
const TENSION_PHRASES: Record<string, Record<string, string>> = {
  b_flame: {
...
function tensionFor(baselineRootId: string, statePathIds: string[]): string | undefined {
 ...
 */
export function route(baselinePath: TreeNode[], statePath: TreeNode[]): Descriptors {
  ...
  for (const node of baselinePath.slice(1)) {
    const d: DescriptorPatch = node.descriptors ?? {};
    ...
    if (d.material) D.material = [d.material[0], D.material[0]].filter(Boolean) as string[]; // swap primary, keep lane's as secondary
